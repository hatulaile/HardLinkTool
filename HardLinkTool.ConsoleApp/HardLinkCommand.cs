...
public sealed class HardLinkCommand : RootCommand
{
    ...
    private readonly Option<int> _refreshTimeOption = new("RefreshTime", "--refresh")
        { Description = "进度刷新时间", DefaultValueFactory = _ => 100 };
        ...
        base("批量生成硬链接工具.")
    {
        ...
        this.SetAction(async (parse, token) =>
        {
            Logger logger = new Logger()
                .AddDebugDisplay(new LocalFileDisplay(@".\debug.log"))
            ...
            if (!parse.GetValue(_noErrorLogFile))
                logger.AddErrorDisplay(new LocalFileDisplay(@".\error.log"));
            ...
            IProgressReport? overwriteDisplay = !parse.GetValue(_noProgressOption)
                ? new OverwriteProgressReport(parse.GetValue(_refreshTimeOption))
                : null;
            var handler = new CreateHardLinkHandler(logger, overwriteDisplay);
            CreateHardLinkResults result = await handler
                .RunAsync(new CreateHardLinkOption(entries, skipSize, isOverwrite), token)
            ...
            if (result.State is CreateHardLinkState.Failed)
            {
                logger.Error($"\n{result.Exception}");
                logger.Fatal($"出现严重错误: 根目录下已输出错误日志 \nMessage: {result.Exception?.Message ?? "NULL"}");
                return (int)ErrorCode.Error;
            }
            ...
            if (result.State is CreateHardLinkState.Canceled)
            {
